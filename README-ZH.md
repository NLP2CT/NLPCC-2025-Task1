...
- [ 2025.04.11 ] 📢 NLPCC-2025 Task 1 的测试数据将于2025年4月11日北京时间下午8:00（UTC+8）前发布，敬请期待！
...
测试数据将于 2025 年 4 月 11 日发布。参赛者需根据测试数据生成结果文件并在2025 年 4 月 20 日及之前提交（在文件中添加一个“label”字段作为预测结果）。主办方将统一进行系统评估，并于 2025 年 4 月 30 日公布所有参赛者的最终得分和排名。同时，主办方还将发布评估答案，便于参赛者验证自己的结果。比赛结果公布后，参赛者可以着手撰写论文，论文提交的截止日期为 2025 年 5 月 22 日。
提交的预测文件必须是一个包含所有文本的单一 JSON 文件。JSON 文件中的每条记录都需包含“text”和“label”两个字段。

提交说明：
- 除测试结果文件外，还需打包提交最终使用的训练数据和代码（附简单readme.md或运行说明文档）。
- 提交材料将用于备份审查。（我们将会对排名靠前的队伍方法和数据进行审核，以保持合理性和公平性）

违规处理：违反参赛规则的队伍将被取消成绩。
...
- 提交内容：测试结果文件、训练数据、源代码（包括readme.md或运行说明文档）。
